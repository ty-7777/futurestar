...
const openChat = (s) => router.push({ path: `/chat/${s.id}`, query: { name: s.sessionName } })
    ...
    <template v-else>
      <van-swipe-cell v-for="s in sessions" :key="s.id">
        <div class="c-item" @click="openChat(s)">
          <van-icon name="chat-o" size="20" class="c-item__icon" />
      ...
      <van-empty v-if="!sessions.length" description="还没有对话，点下方开始咨询" />
    </template>
    <div class="chat-list__new">
      <van-button type="primary" block round icon="plus" @click="onNewChat">新建对话</van-button>
    </div>
  </div>
</template>
